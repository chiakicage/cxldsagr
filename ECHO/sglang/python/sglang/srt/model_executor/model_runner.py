...
NSA_KV_OFFLOAD = os.getenv("NSA_KV_OFFLOAD", None)
NSA_DEV_CACHE_SIZE = os.getenv("NSA_DEV_CACHE_SIZE", "4096")
...
class ModelRunner:
    ...
    def profile_max_num_token(self, total_gpu_memory: int):
        ...
        print(f"{total_gpu_memory=} {available_gpu_memory=} {max_num_token=} {rest_memory=:.2f}GB {cell_size=}")
    ...
    def init_memory_pool(
        ...
        if self.server_args.attention_backend == "ascend":
            ...
            if NSA_KV_OFFLOAD:
                self.token_to_kv_pool_device = MLATokenToKVPool(
                    int(NSA_DEV_CACHE_SIZE),
                    page_size=1,
                    dtype=self.kv_cache_dtype,
                    kv_lora_rank=self.model_config.kv_lora_rank,
                    qk_rope_head_dim=self.model_config.qk_rope_head_dim,
                ...
                self.token_to_kv_pool = NSATokenToKVPoolHost(
                    self.max_total_num_tokens,
                    page_size=self.page_size,
                    dtype=self.kv_cache_dtype,
                    kv_lora_rank=self.model_config.kv_lora_rank,
                    ...
                    end_layer=self.end_layer,
                    index_head_dim=get_nsa_index_head_dim(self.model_config.hf_config),
                    device_pool=self.token_to_kv_pool_device,
    ...
    def forward(
        ...
        # print(f"{forward_batch.seq_lens_cpu=} {forward_batch.extend_seq_lens_cpu=} {forward_batch.forward_mode} {forward_batch.num_token_non_padded=} {self.gpu_id=}")
