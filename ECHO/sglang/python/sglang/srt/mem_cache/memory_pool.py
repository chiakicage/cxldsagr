...
def _view_kv_buffer_as_logical_dtype(
    kv_buffer: torch.Tensor, logical_dtype: torch.dtype
) -> torch.Tensor:
    if kv_buffer.dtype == logical_dtype:
        return kv_buffer
    return kv_buffer.view(logical_dtype)
...
class MLATokenToKVPool(KVCache):
    ...
    def set_mla_kv_buffer(
        ...
        cache_k_rope: torch.Tensor,
        metadata = None,
        host_transfer_req_pool_indices = None,
    ):
        layer_id = layer.layer_id
        ...
        if self.use_nsa and self.nsa_kv_cache_store_fp8:
            ...
            kv_buffer = _view_kv_buffer_as_logical_dtype(
                self.kv_buffer[layer_id - self.start_layer], self.dtype
            )
