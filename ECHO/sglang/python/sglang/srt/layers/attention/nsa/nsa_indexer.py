...
NSA_FUSE_LOGITS_RECALL_EXTEND = get_bool_env_var(
    "SGLANG_NSA_FUSE_LOGITS_RECALL_EXTEND", "false"
)
NSA_FUSE_LOGITS_RECALL = get_bool_env_var(
    "SGLANG_NSA_FUSE_LOGITS_RECALL_DECODE", "false"
)

# should only be set for requests >> 2K
EARLY_EVICT = False

NSA_DUMP_LOGITS = os.getenv("NSA_DUMP_LOGITS")
...
def _get_topk_logits_kernel(
    ...
    topk_idx = tl.load(topk_indices_ptr + q_idx * topk + topk - 1)
    if topk_idx >= 0:
        val = tl.load(logits_ptr + q_idx * logits_stride + topk_idx)
        tl.store(topk_logits_ptr + q_idx, val)

def get_topk_logits(logits: torch.Tensor, topk_indices: torch.Tensor):
    q_len, kv_len = logits.shape
    q_len_, topk = topk_indices.shape
    topk_logits = torch.zeros((q_len,), dtype=logits.dtype, device=logits.device)
    _get_topk_logits_kernel[(q_len,)](
        topk_logits, logits, topk_indices, topk, logits.stride(0)
    )
    return topk_logits
...
class BaseIndexerMetadata(ABC):
    ...
    def topk_transform(
        ...
        Return: Anything, since it will be passed to the attention backend
                ...
                When return_topk_logits is true, also return the logit at
                the last selected raw top-k index for each query.
        """
...
class Indexer(CustomOp):
    def __init__(
        ...
        self.extend_logits_window = 4
        # Keep in sync with DeepGEMM sm90_fp8_mqa_logits_fuse_prefetch.
        MAX_EXTEND_BS = 16
        self.extend_logits_offsets = torch.zeros(
            (MAX_EXTEND_BS,), device="cuda", dtype=torch.float32
        )

        self.decode_logits_ema_decay = 0.5
        self.decode_topk_logits = torch.zeros(
            (MAX_DECODE_BS,), device="cuda", dtype=torch.float32
        )
    ...
    def _get_decode_prefetch_buffers(
        self,
        token_to_kv_pool: Any,
        q_len: int,
        device_pool_buf: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        prefetch_host_loc = token_to_kv_pool.decode_prefetch_host_loc
        prefetch_kv_buf = token_to_kv_pool.decode_prefetch_kv_buf
        assert q_len <= prefetch_host_loc.size(0)
        assert prefetch_kv_buf.size(-1) == device_pool_buf.size(-1)
        return prefetch_host_loc[:q_len], prefetch_kv_buf[:q_len]
    ...
    def _get_topk_paged(
        ...
        use_fused_recall = (
            forward_batch.token_to_kv_pool.device == "cpu" and NSA_FUSE_LOGITS_RECALL
        )

        if use_fused_recall:
            if TYPE_CHECKING:
                assert isinstance(forward_batch.token_to_kv_pool, NSATokenToKVPoolHost)
            # kv data
            device_pool_buf = (
                forward_batch.token_to_kv_pool.device_pool.get_key_buffer(layer_id)
            )
            host_pool_buf = forward_batch.token_to_kv_pool.kv_buffer[layer_id]
            host_token_to_device = forward_batch.token_to_kv_pool.host_token_to_device[
                layer_id
            ]
            q_len = q_fp8.size(0)
            query_recall_counter = forward_batch.token_to_kv_pool.query_recall_counter[
                :q_len
            ]
            query_recall_counter.fill_(0)
            prefetch_host_loc, prefetch_kv_buf = self._get_decode_prefetch_buffers(
                forward_batch.token_to_kv_pool, q_len, device_pool_buf
            )

            logits = deep_gemm.fp8_paged_mqa_logits_fused_v2(
                q_fp8,
                kv_cache_fp8,
                weights,
                seqlens_32,
                ...
                self.decode_topk_logits[:q_len],
                clean_logits=False,
            )
            topk_result, topk_logits = metadata.topk_transform(
                logits, self.index_topk, return_topk_logits=True
            )
            self.decode_topk_logits[:q_len] = (
                self.decode_logits_ema_decay * self.decode_topk_logits[:q_len]
                + (1 - self.decode_logits_ema_decay) * topk_logits
            )
        else:
        ...
        if EARLY_EVICT:
            topk_low_result = metadata.topk_transform(-logits, self.index_topk)
            forward_batch.token_to_kv_pool.early_evict(layer_id, topk_low_result)

        if NSA_DUMP_LOGITS:
            if layer_id in [0, 60]:
                print(f"{layer_id=} {self.step_id=} {logits.shape=} {logits.device=}", flush=True)
            if logits.device.index == 0:
                os.makedirs(NSA_DUMP_LOGITS, exist_ok=True)
                torch.save(logits, f"{NSA_DUMP_LOGITS}/layer{layer_id}_step{self.step_id}.pt")
            self.step_id += 1
    ...
    def _get_topk_ragged(
        ...
        if TYPE_CHECKING:
            assert isinstance(forward_batch.token_to_kv_pool, NSATokenToKVPool)
        if os.getenv("FAKE_P_NODE") is not None:
            fake_topk = torch.full(
                (q_fp8.shape[0], self.index_topk), -1, dtype=torch.int32, device="cuda"
            )
            fake_topk[0, :] = -1
            return fake_topk
        ...
        block_tables = metadata.get_page_table_64()
        page_table_1 = metadata.get_page_table_1()
        extend_seq_to_req = metadata.get_extend_seq_to_req()
        ...
        if forward_batch.token_to_kv_pool.device == "cpu" and NSA_FUSE_LOGITS_RECALL_EXTEND:
            assert forward_batch.batch_size <= self.extend_logits_offsets.size(0)
        ...
        for i in range(forward_batch.batch_size):
            ...
            if extend_seq_len == forward_batch.seq_lens_cpu[i]:     # first chunked prefill batch
                self.extend_logits_offsets[i].fill_(0.)
        ...
        # print(f"{page_table_1=} {ks=} {ke=}", flush=True)

        if forward_batch.token_to_kv_pool.device == "cpu" and NSA_FUSE_LOGITS_RECALL_EXTEND:
            if TYPE_CHECKING:
                assert isinstance(forward_batch.token_to_kv_pool, NSATokenToKVPoolHost)
            # kv data
            device_pool_buf = forward_batch.token_to_kv_pool.device_pool.get_key_buffer(layer_id)
            host_pool_buf = forward_batch.token_to_kv_pool.kv_buffer[layer_id]
            device_pool_loc_alloc_buf = forward_batch.token_to_kv_pool.device_pool_loc_alloc_buf
            device_pool_loc_alloc_buf.fill_(0)
            device_pool_allocator = forward_batch.token_to_kv_pool.device_pool_allocator[layer_id]
            # metadata
            device_pool_priority = forward_batch.token_to_kv_pool.device_pool_priority[layer_id]
            # 0 should not be allocated
            device_pool_loc_small_priority = torch.argsort(device_pool_priority, descending=False)
            device_pool_loc_small_priority = device_pool_loc_small_priority.to(torch.int32)

            device_token_to_host = forward_batch.token_to_kv_pool.device_token_to_host[layer_id]
            host_token_to_device = forward_batch.token_to_kv_pool.host_token_to_device[layer_id]
            recall_counter = forward_batch.token_to_kv_pool.recall_counter
            recall_counter.fill_(0)

            logits = deep_gemm.fp8_mqa_logits_fuse_prefetch(
                q_fp8,
                kv_fp8,
                weights,
                ks,
            ...
            # self.extend_logits_offsets.fill_(torch.mean(logits[-self.extend_logits_window:]))
            offset = 0
            for i in range(forward_batch.batch_size):
                s = max(offset + extend_seq_lens_cpu[i]-self.extend_logits_window, offset)
                self.extend_logits_offsets[i].fill_(torch.mean(
                    logits[s: offset + extend_seq_lens_cpu[i]]
                ))
                offset += extend_seq_lens_cpu[i]

            device_pool_allocator.post_alloc(device_pool_loc_alloc_buf)
            forward_batch.token_to_kv_pool.update_priority_when_use(layer_id, device_pool_loc_alloc_buf)
        else:
