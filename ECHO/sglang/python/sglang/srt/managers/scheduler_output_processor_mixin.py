                ...
                if self.page_size == 1:
                    ...
                    if batch.spec_algorithm.is_eagle():
                        ...
                        if (
                            ...
                            indices_to_free = batch.out_cache_loc[i : i + 1]
                        # for overlap schedule, the last decode step is freed here
                        # and for paged allocator, only free when this token occupies a new page
                        # for token-wise device pool allocator, it should be freed explicitly

                if indices_to_free is not None:
                    self.token_to_kv_pool_allocator.free(indices_to_free)
                else:
                    from sglang.srt.mem_cache.memory_pool_host import NSATokenToKVPoolHost
                    if isinstance(self.token_to_kv_pool_allocator._kvcache, NSATokenToKVPoolHost):
                        self.token_to_kv_pool_allocator._kvcache.free_req_device_pool(
