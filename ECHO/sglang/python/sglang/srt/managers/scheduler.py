...
class Scheduler(
    ...
    def __init__(
        ...
        if self.pp_rank == 0 and self.attn_tp_rank == 0:
            ...
            # only create when using nsa load balance
            self.send_to_dp_controller = None
            if port_args.dp_controller_ipc_name and server_args.load_balance_method == "nsa_max_device":
    ...
    def run_batch(
        ...
        if self.is_generation:
            ...
            if self.enable_overlap:
                ...
                if self.send_to_dp_controller:
                    device_avai_size = self.model_worker.get_device_avai_size()
                    self.send_to_dp_controller.send_pyobj(
                        DPDeviceSpaceUpdReq(dp_rank=self.dp_rank, device_available_size=device_avai_size)
                    )
