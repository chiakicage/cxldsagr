...
class TpModelWorker(BaseTpWorker):
    ...
    def get_device_avai_size(self):
        assert self.model_runner.token_to_kv_pool.device == "cpu"
        device_pool_allocator = self.model_runner.token_to_kv_pool.device_pool_allocator
        avai_size = torch.zeros((1,), device=device_pool_allocator[0].device, dtype=torch.int32)
        for allocator in device_pool_allocator:
            avai_size += allocator.available_size()
        return avai_size.item()
