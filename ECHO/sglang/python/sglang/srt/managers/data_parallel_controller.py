...
class DPDeviceSpace:
    def __init__(self, dp_size: int):
        ...
        self.dp_rank_device_space = [0] * self.dp_size

    def update(self, space_update: DPDeviceSpaceUpdReq):
        self.dp_rank_device_space[space_update.dp_rank] = space_update.device_available_size

    def dispatch(self):
        if all([space == 0 for space in self.dp_rank_device_space]):
            return None
        else:  
            sorted_space, index = zip(*sorted(zip(self.dp_rank_device_space, range(self.dp_size))))
            top1 = sorted_space[-1]
            top2 = sorted_space[-2]
            if (top1 - top2) / top2 > 0.1:
                return index[-1]
            else:
                return None
...
class DataParallelController:
    ...
    def __init__(self, server_args: ServerArgs, port_args: PortArgs) -> None:
        ...
        if server_args.node_rank == 0:
            ...
            self.recv_from_scheduler = get_zmq_socket(
                self.context, zmq.PULL, port_args.dp_controller_ipc_name, True
            )
        ...
        dispatch_lookup = {
            ...
            LoadBalanceMethod.NSA_MAX_DEVICE: self.nsa_max_device_space_scheduler,
        }
        self.dispatching = dispatch_lookup[self.load_balance_method]

        # Load balance budget
        self.dp_budget = DPBudget()
        self.dp_device_space = DPDeviceSpace(server_args.dp_size)
    ...
    def handle_device_space_update_req(self, obj):
        self.dp_device_space.update(obj)
    ...
    def init_dispatcher(self):
        self._request_dispatcher = TypeBasedDispatcher(
            [
                ...
                (DPDeviceSpaceUpdReq, self.handle_device_space_update_req),
            ]
        )
    ...
    def nsa_max_device_space_scheduler(self, req):
        if self.maybe_external_dp_rank_routing(req):
            return
        target_worker = self.dp_device_space.dispatch()
        if target_worker is None:
            self.round_robin_scheduler(req)
        else:
            self.workers[target_worker].send_pyobj(req)
