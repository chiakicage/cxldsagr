...
def get_dataset(args, tokenizer, model_id=None):
    ...
    if args.dataset_name == "sharegpt":
        ...
        input_requests = sample_infinitebench(
            ...
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            fixed_output_len=args.infini_out,
            infinitebench_task=args.infinitebench_task,
            infini_min_context=args.infini_min,
            infini_max_context=args.infini_max,
            prompt_suffix=args.prompt_suffix,
            apply_chat_template=args.apply_chat_template,
        ...
        input_requests = sample_longbench(
            ...
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            fixed_output_len=args.longbench_out,
            longbench_min_context=args.longbench_min,
            longbench_max_context=args.longbench_max,
            prompt_suffix=args.prompt_suffix,
            apply_chat_template=args.apply_chat_template,
            sort_len=args.sort_len,
        )
...
def sample_sharegpt_requests(
    ...
    for i in range(len(dataset)):
        ...
        if min_len and prompt_len + output_len < min_len:
...
def sample_mixed_requests(
    ...
    long_request_dataset = sample_infinitebench(
        dataset_path,
        num_requests,
        tokenizer,
        fixed_output_len,
    ...
    short_request_dataset = sample_sharegpt_requests(
        tokenizer,
        fixed_output_len,
...
def sample_infinitebench(
    ...
    def _resolve_local_dataset_files(path_str: str) -> List[Path]:
        path = Path(path_str)
        if not path.exists():
            raise ValueError(f"InfiniteBench dataset path does not exist: {path}")

        if path.is_dir():
            if infinitebench_task:
                task_path = path / f"{infinitebench_task}.jsonl"
                if not task_path.exists():
                    raise ValueError(
                        f"InfiniteBench task file not found: {task_path}"
                    )
                return [task_path]

            task_files = sorted(path.glob("*.jsonl"))
            if not task_files:
                raise ValueError(f"No JSONL files found under {path}")
            return task_files

        if infinitebench_task and path.stem != infinitebench_task:
            raise ValueError(
                f"InfiniteBench task '{infinitebench_task}' does not match dataset file "
                f"'{path.name}'. Pass the matching task file or a task directory instead."
            )
        return [path]

    def _get_prompt_and_output_len(row: Dict[str, Any]) -> Tuple[str, int]:
        base_row = row.get("source") if isinstance(row.get("source"), dict) else row
        if "context" not in base_row or "input" not in base_row:
            raise ValueError(
                "InfiniteBench rows must contain 'context' and 'input', either at the "
                "top level or under a 'source' object."
            )

        prompt = base_row["context"] + "\n" + base_row["input"]
        output_len = fixed_output_len
        if output_len is None:
            candidate = row.get("completion_tokens")
            if candidate is None and isinstance(row.get("meta_info"), dict):
                candidate = row["meta_info"].get("completion_tokens")
            output_len = candidate if candidate is not None else 256
        return prompt, int(output_len)

    if dataset_path:
        all_dataset = []
        for file_path in _resolve_local_dataset_files(dataset_path):
            all_dataset.extend(_load_local_rows(file_path))
    else:
        if infinitebench_task:
            dataset = load_dataset(
                "xinrongzhang2022/InfiniteBench", infinitebench_task, features=ft
            )
            if "train" in dataset:
                all_dataset = list(dataset["train"])
            else:
                all_dataset = list(dataset)
        else:
    ...
    for i in range(len(all_dataset)):
        ...
        # Tokenize the prompts and completions.
        prompt, output_len = _get_prompt_and_output_len(all_dataset[i])
...
def get_gen_prefix_cache_path(args, tokenizer):
    ...
    cache_key = (
        f"{'interleave_' if args.gsp_interleave else ''}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="The context length of the model for the ShareGPT dataset. Requests shorter than the context length will be dropped.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--infinitebench-task",
        "--infini-task",
        dest="infinitebench_task",
        type=str,
        default=None,
        help="Specific InfiniteBench task to benchmark. Works with a task directory or the HuggingFace InfiniteBench configs.",
    )
    parser.add_argument(
        "--longbench-max",
        type=int,
    ...
    group.add_argument(
        "--gsp-interleave",
        action="store_true",
        help="Interleave the requests order for generated-shared-prefix dataset",
    )
    group.add_argument(
        "--gsp-shuffle",
        action="store_true",
