...
def test_topk_transform_kernel(bs: int, k: int, seq_len: int) -> None:
    ...
    dst_page_table_our_with_logits, topk_logits_our = fast_topk_transform_fused(
        score=score,
        lengths=lengths,
        page_table_size_1=src_page_table,
        cu_seqlens_q=cu_seqlens_q,
    ...
    topk_logits_ref = score.gather(
        1, dst_page_table_our_with_logits[:, -1:].to(torch.long)
    ).squeeze(1)

    assert torch.equal(topk_logits_our, topk_logits_ref)

    # sort and compare
    dst_page_table_our = torch.sort(dst_page_table_our, dim=-1).values
    dst_page_table_our_with_logits = torch.sort(
        dst_page_table_our_with_logits, dim=-1
    ).values
    ...
    assert_equal(
        score, dst_page_table_ref, dst_page_table_our_with_logits, bs, k, seq_len
    )
