...
TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {
  ...
  m.def(
      ...
      "cu_seqlens_q, Tensor? topk_logits=None) -> ()");
  m.impl("fast_topk_transform_fused", torch::kCUDA, &fast_topk_transform_interface);
  m.def("fast_argtopk_m2048(Tensor score, Tensor indices, Tensor topk, bool largest=True) -> ()");
  m.impl("fast_argtopk_m2048", torch::kCUDA, &fast_argtopk_m2048_interface);
  m.def("fast_argtopk(Tensor score, Tensor indices, Tensor topk, bool largest=True) -> ()");
  m.impl("fast_argtopk", torch::kCUDA, &fast_argtopk_interface);
  m.def("fast_argmin_bounded(Tensor score, Tensor indices, Tensor topk) -> ()");
  m.impl("fast_argmin_bounded", torch::kCUDA, &fast_argmin_bounded_interface);
