...
class SMXXFP8PagedMQALogitsFusedRuntimeV2 final: public LaunchRuntime<SMXXFP8PagedMQALogitsFusedRuntimeV2> {
public:
    struct Args {
        int batch_size;
        int next_n;
        ...
        int* prefetch_host_loc;
        at::BFloat16* prefetch_kv_buf;
        int* host_token_to_device;
        uint32_t* query_recall_counter;
        float* decode_topk_logits;
...
static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&sm{}_fp8_paged_mqa_logits_fused_v2<
        {}, {},
        {}, {}, {},
        {}, {}, {},
        {},
    ...
    args.next_n, args.num_heads,
    args.head_dim, args.mla_head_dim, args.block_kv,
    args.num_q_stages, args.num_kv_stages, args.num_pf_stages,
    args.split_kv,
    args.num_specialized_threads, args.num_math_threads, args.num_prefetch_threads);
    }
    ...
    static void launch_impl(const KernelHandle& kernel, const LaunchConfigHandle& config, Args args) {
        DG_CUDA_UNIFIED_CHECK(launch_kernel(kernel, config,
            ...
            static_cast<uint64_t>(args.logits_stride),
            static_cast<uint64_t>(args.block_table_stride),
            static_cast<uint64_t>(args.page_table_1_stride),
            args.context_lens, args.logits,
            args.block_table, args.schedule_meta,
            args.page_table_1,
            args.device_pool_buf,
            args.host_pool_buf,
            args.prefetch_host_loc,
            args.prefetch_kv_buf,
            args.host_token_to_device,
            args.query_recall_counter,
            args.decode_topk_logits,
...
static void smxx_fp8_paged_mqa_logits_fused_v2(const torch::Tensor& q,
                                            const torch::Tensor& kv_cache,
                                            const torch::Tensor& kv_cache_scales,
                                            const torch::Tensor& weights,
                                            const torch::Tensor& context_lens,
    ...
    const int num_specialized_threads = 128;
    const int num_math_threads = num_math_warp_groups * 128;
    const int num_prefetch_threads = 128;
    const int num_extra_threads = device_runtime->get_arch_major() == 10 ? 128 : 0;
    const int num_q_stages = 3, num_kv_stages = 3;
    const int split_kv = num_math_warp_groups * block_kv;
    DG_HOST_ASSERT(logits_stride % (num_math_warp_groups * block_kv) == 0);
    ...
    // The v2 prefetch branch also has a small static shared-memory scratch.
    // Reserve it before maximizing the dynamic logits pipeline stages.
    constexpr int smem_capacity = SM90ArchSpec::smem_capacity < SM100ArchSpec::smem_capacity ?
        SM90ArchSpec::smem_capacity : SM100ArchSpec::smem_capacity;
    const int static_prefetch_smem_size =
        align(num_prefetch_threads * static_cast<int>(sizeof(int)), swizzle_alignment);
    const int fixed_dynamic_smem_size =
        smem_q_pipe_size +
        num_math_warp_groups * smem_kv_pipe_size +
        smem_umma_barriers +
        smem_tmem_ptr;
    const int pf_logits_size_per_stage = next_n * num_math_warp_groups * block_kv * static_cast<int>(sizeof(float));
    constexpr int pf_barrier_size_per_stage = 2 * 8;
    DG_HOST_ASSERT(smem_capacity > fixed_dynamic_smem_size + static_prefetch_smem_size);
    int num_pf_stages =
        (smem_capacity - static_prefetch_smem_size - fixed_dynamic_smem_size) /
        (pf_logits_size_per_stage + pf_barrier_size_per_stage);
    while (fixed_dynamic_smem_size +
           num_pf_stages * pf_logits_size_per_stage +
           align(num_pf_stages * pf_barrier_size_per_stage, swizzle_alignment) +
           static_prefetch_smem_size > smem_capacity) {
        --num_pf_stages;
    }
    DG_HOST_ASSERT(num_pf_stages >= 3);

    const int smem_pf_pipe_size =
        num_pf_stages * pf_logits_size_per_stage +
        align(num_pf_stages * pf_barrier_size_per_stage, swizzle_alignment);
    const int smem_size = fixed_dynamic_smem_size + smem_pf_pipe_size;
    DG_HOST_ASSERT(smem_size + static_prefetch_smem_size <= SM90ArchSpec::smem_capacity);
    DG_HOST_ASSERT(smem_size + static_prefetch_smem_size <= SM100ArchSpec::smem_capacity);

    // Launch
    const SMXXFP8PagedMQALogitsFusedRuntimeV2::Args& args = {
        .batch_size = batch_size,
        .next_n = next_n,
        .num_heads = num_heads,
        .head_dim = head_dim,
        ...
        .prefetch_host_loc = prefetch_host_loc.data_ptr<int>(),
        .prefetch_kv_buf = prefetch_kv_buf.data_ptr<at::BFloat16>(),
        .host_token_to_device = host_token_to_device.data_ptr<int>(),
        .query_recall_counter = query_recall_counter.data_ptr<uint32_t>(),
        .decode_topk_logits = decode_topk_logits.data_ptr<float>(),
        .device_pool_size_m1 = device_pool_size_m1,
        .mla_head_dim = mla_head_dim,
        .launch_args = LaunchArgs(num_sms,
                                   num_specialized_threads + num_math_threads + num_prefetch_threads + num_extra_threads,
                                   smem_size)
    };
    const auto& code = SMXXFP8PagedMQALogitsFusedRuntimeV2::generate(args);
    const auto& runtime = compiler->build("sm90_fp8_paged_mqa_logits_fused_v2", code);
    SMXXFP8PagedMQALogitsFusedRuntimeV2::launch(runtime, args);
}
