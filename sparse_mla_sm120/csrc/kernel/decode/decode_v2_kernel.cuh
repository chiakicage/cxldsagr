    ...
    __grid_constant__ const DecodeV2ColdParams cold)
{
    ...
    // Keep each role in a disjoint control-flow region so ptxas can prove
    // which dynamic register limit dominates the complete request loop.
    if (wy == 2) {
        asm volatile("setmaxnreg.dec.sync.aligned.u32 %0;\n" :: "n"(32));
        const DecodingSchedMeta meta = sched_meta[partition_idx];

        // ── IO batch loop driven by scheduler metadata ──────────────
        for (int req = meta.begin_req_idx; req <= meta.end_req_idx && req < num_batches; req++) {
            ...
            // Both role loops evaluate the same request range and reach
            // barrier 3 together when the mbarriers are recycled.
        ...
        // ── Math batch loop driven by scheduler metadata ────────────
        for (int req = meta.begin_req_idx; req <= meta.end_req_idx && req < num_batches; req++) {
            const int s_i = req * s_q + s_q_idx;

            // Keep this setup identical to the IO loop so all full-CTA
            // barriers are reached in the same order by every warpgroup.
            const int topk_len = cold.topk_length ? __ldg(cold.topk_length + req) : topk;
            const int orig_topk_padded = max(((topk_len + BI - 1) / BI) * BI, BI);
            const int num_orig_blocks = orig_topk_padded / BI;
            const int extra_topk_len = (cold.extra_topk > 0)
        ...
        // ── Epilogue ────────────────────────────────────────────────
        // These values are epilogue-only. Keeping them out of the tile loop
        // avoids extending their live ranges across the register-heavy math.
