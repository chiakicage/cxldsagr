...
bool is_cuda_or_pinned_cpu(const torch::Tensor& tensor) {
    return tensor.is_cuda() || (tensor.device().is_cpu() && tensor.is_pinned());
}

void check_cuda_or_pinned_cpu(const torch::Tensor& tensor, const char* name) {
    TORCH_CHECK(
        is_cuda_or_pinned_cpu(tensor),
        name, " must be a CUDA tensor or a pinned CPU tensor");
}

}  // namespace

// Forward declarations — combine v2
    ...
    bool bf16_qk)
{
    ...
    check_cuda_or_pinned_cpu(KV_cache, "KV_cache");
    if (extra_k_cache.has_value()) {
        check_cuda_or_pinned_cpu(*extra_k_cache, "extra_k_cache");
    }
